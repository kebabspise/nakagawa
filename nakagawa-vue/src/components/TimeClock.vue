...
const createWorkLog = async (data) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(`サーバーエラー: ${response.status} - ${errorData}`)
  }
...
const updateLatestWorkLog = async (userId, endTime) => {
  try {
    ...
    console.log('取得した勤務記録:', workLogs) // デバッグ用
    
    // 日本時間での今日の日付を取得
    const today = new Date(endTime.getTime() + (9 * 60 * 60 * 1000)) // UTC+9
    ...
    const incompleteTodayLogs = workLogs.filter(log => {
      ...
      console.log(`ログID ${log.id}: 日付=${logDateStr}, work_end=${log.work_end}`) // デバッグ用
      
      return logDateStr === today && (log.work_end === null || log.work_end === undefined)
    })
    
    console.log('未完了の今日の記録:', incompleteTodayLogs) // デバッグ用
    ...
    console.log('更新対象のログ:', latestLog) // デバッグ用
    ...
    if (!updateResponse.ok) {
      ...
      throw new Error(`退勤記録の更新に失敗しました: ${updateResponse.status}`)
    }
